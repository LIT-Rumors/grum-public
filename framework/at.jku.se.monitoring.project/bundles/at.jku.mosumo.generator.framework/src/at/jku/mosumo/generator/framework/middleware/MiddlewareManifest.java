...
public class MiddlewareManifest extends AbstractJavaSource implements FileSourceCommand {

	public MiddlewareManifest(IPackageFragmentRoot packageFragmentRoot, MOConfig moConfig, IProgressMonitor monitor) {
	...
	public void createFileSource() {
		...
		maniContent.append("Bundle-ManifestVersion: 2\n")
				...
				.append("Bundle-SymbolicName: " + getPackageFragmentRoot().getJavaProject().getProject().getName()
						+ "; singleton:=true\n")
		...
		System.out.println(getMoConfig().getSystem().getEClass().getName());

		if (getMoConfig().getSystem().getEClass().getInstanceClass() != null
				&& getMoConfig().getSystem().getEClass().getInstanceClass().getClassLoader() != null) {
			// requiredBundles.add((((EquinoxClassLoader)getMoConfig().getSystem().getEClass().getInstanceClass().getClassLoader()).getBundle().getSymbolicName()));
			// TODO: ---CLASSNAMING--- needs fixing...
		}
		String projectName = MonitoringGeneratorUtils.findProjectName(getMoConfig());

		requiredBundles.add((projectName));

		System.out.println(getMoConfig().getSystem().getEClass().getEPackage());
		...
		write(getPackageFragmentRoot().getJavaProject().getProject().getFolder("META-INF"), "MANIFEST.MF",
	...
	public static final List<String> defaultBundles = Arrays.asList("at.jku.se.mosumo.monitoring.util",
			"at.jku.se.mosumo.tools.logging", "org.eclipse.paho.client.mqttv3", "org.emfjson.jackson",
			"com.fasterxml.jackson.core.jackson-databind", "org.eclipse.emf.ecore.xmi",
			"org.eclipse.viatra.query.runtime", "com.fasterxml.jackson.core.jackson-databind",
			"com.fasterxml.jackson.core.jackson-core", "at.jku.se.mosumo.mm", "org.apache.log4j",
			"org.apache.logging.log4j", "com.fasterxml.jackson.core.jackson-annotations");
